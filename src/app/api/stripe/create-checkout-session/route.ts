...
export async function POST(req: Request) {
  ...
  try {
    const session = await stripe.checkout.sessions.create(
      {
        //all the prices that have one time payment products
        line_items: prices.map((price) => ({
          price: price.productId,
          quantity: 1,
        })),

        //if there are subscription Prices, we use subscription_data intent
        ...
        //if not, we use subscription_data intent
        ...(!subscriptionPriceExists && {
          payment_intent_data: {
