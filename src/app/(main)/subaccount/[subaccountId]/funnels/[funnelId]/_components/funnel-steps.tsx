...
/* This component is 
 responsible for displaying and managing the list of funnel pages. 
 It provides drag-and-drop functionality to reorder the pages and a modal to create new pages.
*/
const FunnelSteps = ({ funnel, funnelId, pages, subaccountId }: Props) => {
  
  const [clickedPage, setClickedPage] = useState<FunnelPage | undefined>(
    pages[0] //Tracks the currently selected page. (First page by default)
  );
  const { setOpen } = useModal();
  const [pagesState, setPagesState] = useState(pages); //Manages the state of the pages, including their order.

  //This function captures the page being dragged. It retrieves the draggableId from the event and finds the corresponding page in the pagesState.
  const onDragStart = (event: DragStart) => {
    //current chosen page
    const { draggableId } = event;
    const value = pagesState.find((page) => page.id === draggableId);
  };
  //Handles the logic for reordering pages when a drag-and-drop action is completed.
  const onDragEnd = (dropResult: DropResult) => {
    ...
    //change state
    //If the drop destination is valid and different from the source, it updates the page order
    const newPageOrder = [...pagesState]
      // toSpliced works similarly to splice, but it returns a new array instead of modifying the original one.
      //array.toSpliced(start, deleteCount, ...items)
      .toSpliced(source.index, 1) // removes the item from the source.index.which is the index of the item being dragged. 1: Remove one element from the source.index
      .toSpliced(destination.index, 0, pagesState[source.index]) //destination.index: The index where the item should be inserted. 0: Remove zero elements(just an insertion). pagesState[source.index]: The item being dragged.
      .map((page, idx) => { // iterates through the new array and updates the order property of each page to reflect its new position.
  ...
  return (
    <AlertDialog>
      <div className="flex border-[1px] lg:!flex-row flex-col ">
        ...
        {/* settings for that page -right side */}
