...
// Declare variables to store currency, session data, net income, potential income, and closing rate.
const SubaccountPageId = async ({ params, searchParams }: Props) => {
  ...
  // Fetch subaccount details from the database using the subaccountId.
  ...
  const startDate = new Date(`${currentYear}-01-01T00:00:00Z`).getTime() / 1000 // Calculate the start date of the current year in Unix timestamp format.
  const endDate = new Date(`${currentYear}-12-31T23:59:59Z`).getTime() / 1000 // Calculate the end date of the current year in Unix timestamp format.

  // If no subaccount details are found, return early
  if (!subaccountDetails) return

  //Retrieve the Stripe account details using the connectAccountId.
  if (subaccountDetails.connectAccountId) {
    const response = await stripe.accounts.retrieve({
      stripeAccount: subaccountDetails.connectAccountId,
    })
    //Set the currency to the account's default currency or 'MYR' if not available.
    currency = response.default_currency?.toUpperCase() || 'MYR'
    //Fetch the list of checkout sessions within the date range.
    ...
    //Map the fetched sessions to include a formatted date and total amount in the correct format.
    ...
    //Filter and map sessions with status 'complete'.
    ...
    //Filter and map sessions with status 'open' or 'expired'.
    ...
    //Calculate the total closed session amount.
      ...
      <div className="relative h-full">
        ...
        <div className="flex flex-col gap-4 pb-6">
          ...
          <div className="flex gap-4 xl:!flex-row flex-col">
            <Card className="p-4 flex-1 h-[450px] overflow-auto custom-scroll relative">
              <CardHeader>
